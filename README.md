...
│   └── worker.ts                 # Cloudflare Worker 入口（浏览量 API、健康检查）
...
文章浏览量使用自部署 Umami：Cloudflare Worker 在服务端登录 Umami API（`POST /api/auth/login`）后查询 `/api/websites/{websiteId}/stats`。`UMAMI_HOST` 与 `UMAMI_WEBSITE_ID` 是 `wrangler.jsonc` 中的公开 vars（tracker 标签本身就含 website id）；`UMAMI_USERNAME`、`UMAMI_PASSWORD`、`HEALTH_CHECK_TOKEN` 是 Worker secret，生产环境不要把真实值写入 `.env`、`wrangler.jsonc` 或示例文件，部署前运行：
...
- 浏览量：详情页通过 `src/scripts/view-counter.js` 请求 `/api/views/{slug}`；Cloudflare Worker 在服务端登录自部署 Umami 后查询文章路径的 pageviews，失败时前端自动隐藏浏览量。
