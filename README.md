...
这是一个基于 [Astro](https://astro.build) 的静态个人站点，已从根目录 HTML/CSS/vanilla JS 全面迁移完成。Phase 0-8 已完成：博客、作品、工具和更新日志由 Astro 内容集合驱动，所有页面已转为 Astro 组件，RSS feed、sitemap、giscus 评论区、文章字数/阅读时间、归档页、MiniSearch 懒加载搜索、Worker 健康检查、文章浏览量 Worker 代理、工具页 PWA、图标 WebP 降级和 UI 国际化均已完成。Phase 4 清理已执行，旧管线文件已移除。
...
Phase 7 后，文章页搜索不再内联完整搜索数组。`/search-index.json` 在构建时由 `src/lib/search-index-builder.ts` 生成 MiniSearch 序列化索引，客户端首次搜索时由 `src/lib/search-client.ts` 懒加载。搜索状态继续写入 URL 参数（`q`、`category`、`tag`），便于返回文章列表时恢复。
