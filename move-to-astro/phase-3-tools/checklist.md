...
## 导航调整
- [ ] Header 导航栏不包含"工具"项（仅首页、作品、文章、关于）
- [ ] 移除后所有页面导航高亮正确
...
## 工具集区域（ToolsSection）
- [ ] works.astro 页面在作品卡片下方显示"工具集"区域
- [ ] 三个选项卡（在线计时器 / 随机选择器 / Markdown 渲染工具）切换正常
- [ ] 切换选项卡计时器状态保持一致
...
## 格式化 HTML 输出
- [ ] 格式化 HTML 按钮正常工作
- [ ] 格式化后的 HTML 缩进正确（块级元素独占一行并缩进，内联元素保留在行内）
- [ ] 格式化导出下载正常
- [ ] 格式化复制到剪贴板正常
...
- [ ] 微信公众号格式按钮正常工作
- [ ] 微信格式预览区显示正确（内联样式生效）
- [ ] 微信格式下复制功能正常（复制内联样式 HTML）
- [ ] 微信格式 HTML 可直接粘贴到微信公众号编辑器
- [ ] 普通模式和微信格式模式切换正常，不丢失输入内容
- [ ] 微信格式样式规则正确：正文 15px/1.8 行高/#3f3f3f 色、标题加粗、代码块灰底、引用左边框、图片居中、表格细边框

## 样式与质量
- [ ] 工具页专属样式不污染其他页面
- [ ] MarkdownToolWidget 组件可在 works.astro 内和独立页面两种场景复用
